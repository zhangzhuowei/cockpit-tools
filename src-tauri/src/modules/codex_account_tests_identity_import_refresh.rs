...
use super::{
    ...
    clear_retired_app_server_preflight_reauth, decode_jwt_payload_value,
    detect_auth_file_plan_type_from_path, ensure_managed_account_fresh,
    extract_codex_import_candidate_from_value, extract_codex_tokens_from_value, extract_user_info,
    ...
    read_quick_config_from_config_toml, remove_accounts, resolve_api_provider_config, save_account,
    ...
    write_quick_config_to_config_toml_with_default, ApiProviderConfig, CodexAccessTokenImportHints,
...
fn parses_and_projects_agent_identity_auth_json() {
    ...
    let account =
        super::build_agent_identity_account_draft(identity).expect("build Agent Identity account");
...
fn parses_sub2api_agent_identity_export_file_with_duplicate_account_fields() {
    ...
    let values =
        super::codex_batch_import_values_from_content(&content).expect("parse Sub2API export file");
...
fn agent_identity_members_in_the_same_workspace_coexist() {
    ...
    let second = upsert_agent_identity_account(build("user-b", "second@example.com", "runtime-b"))
        .expect("import second workspace member");
    let updated_first =
        upsert_agent_identity_account(build("user-a", "updated@example.com", "runtime-a-updated"))
...
fn agent_identity_legacy_storage_id_is_reused_only_for_matching_member() {
    ...
    let updated = upsert_agent_identity_account(identity.clone()).expect("reimport legacy account");
...
fn agent_identity_cannot_be_used_as_api_key_oauth_binding() {
    ...
    let error = super::validate_api_key_bound_oauth_account(&api_key_account, &agent_account.id)
...
fn portable_api_key_import_projects_its_own_relay_credentials() {
    ...
    write_account_bundle_to_dir(&profile_dir, &imported).expect("project imported API key account");
    ...
    let config = fs::read_to_string(profile_dir.join("config.toml")).expect("read imported config");
...
fn clears_only_retired_app_server_preflight_reauth_state() {
    ...
    affected.reauth_reason =
        Some("官方 app-server 返回 invalid_refresh_token，账号无法切换，请重新授权".to_string());
...
#[test]
fn token_refresh_file_lock_respects_custom_timeout() {
    let _lock = crate::modules::test_support::env_lock()
        .lock()
        .unwrap_or_else(|err| err.into_inner());
    let env = TestEnvGuard::new("codex-token-refresh-lock-timeout-test");
    let path = super::codex_token_refresh_file_lock_path("codex-account-id");
    ...
    let error = match result {
        Ok(_) => panic!("lock acquisition should time out quickly"),
        Err(error) => error,
    };
...
fn instance_launch_preflight_uses_local_credentials_without_internal_config_request() {
    ...
    let lock_path = super::codex_token_refresh_file_lock_path(&account.id);
    fs::create_dir_all(&lock_path).expect("occupy refresh lock path");
...
fn upsert_access_token_only_account_uses_access_claims() {
    ...
    let candidate =
        extract_codex_import_candidate_from_value(&serde_json::Value::String(access_token.clone()))
...
fn import_pending_oauth_delimited_line_creates_pending_account() {
    ...
    assert_eq!(
        ...
        Some("https://mail.example.test/open.php?mail=user%2Btag%40example.com&pwd=secret&limit=5")
    );
...
fn switch_auth_error_does_not_claim_api_only_when_api_was_rejected() {
    ...
    account.reauth_reason =
        Some("Codex 登录授权已被服务端撤销，无法自动刷新。请重新登录 Codex 账号。".to_string());
    account.quota_error = Some(crate::models::codex::CodexQuotaErrorInfo {
        code: None,
        message: "API 返回错误 401 Unauthorized [body_len:22]".to_string(),
        timestamp: now_timestamp(),
...
async fn clearing_client_auth_observation_preserves_credentials_and_reauth_state() {
    ...
    assert!(clear_client_auth_observation(&account.id)
        .await
...
fn runtime_prepare_projects_expired_id_token_when_access_token_is_valid() {
    ...
    assert_eq!(prepared.id, account.id);
    let projected = fs::read_to_string(profile_dir.join("auth.json")).expect("read projected auth");
...
fn missing_refresh_token_reauth_is_cleared_for_access_token_only_accounts() {
    ...
    account.reauth_reason = Some(
        "Codex 登录授权缺少 refresh_token，无法自动续期；当前 access_token 已不可用。".to_string(),
    );
...
fn runtime_authority_sync_writes_only_the_newest_running_profile() {
    ...
    assert!(
        super::sync_account_from_runtime_authority_dirs(&account.id, &[older_dir, newer_dir])
...
fn reauth_consumer_sync_keeps_running_official_profile_unchanged() {
    ...
    let persisted_runtime = super::load_local_oauth_snapshot_from_official_store(&env.codex_home())
