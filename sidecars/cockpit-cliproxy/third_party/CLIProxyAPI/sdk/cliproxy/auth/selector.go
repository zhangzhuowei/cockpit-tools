...
func (s *SessionAffinitySelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	...
	modelKey := canonicalModelKey(model)
	cacheKey := sessionAffinityCacheKey(provider, primaryID, modelKey, opts.Metadata)
	isFork := false
	if opts.Metadata != nil {
	...
	if fallbackID != "" && fallbackID != primaryID {
		fallbackKey = sessionAffinityCacheKey(provider, fallbackID, modelKey, opts.Metadata)
	}
...
// sessionAffinityCacheKey keeps explicit session bindings isolated by caller scope.
// A sidecar can serve multiple downstream API keys at once; sharing a binding
// across those keys can route a restricted key to an account it cannot use.
// Preserve the legacy key shape when no scope is present so existing callers
// and persisted in-memory bindings remain compatible.
func sessionAffinityCacheKey(namespace, sessionID, model string, metadata map[string]any) string {
	base := namespace + "::" + sessionID + "::" + model
	callerScope := sessionMetadataString(metadata, cliproxyexecutor.CallerScopeMetadataKey)
	if callerScope == "" {
		return base
	}
	return base + "::caller_scope::" + callerScope
}
...
func (s *SessionAffinitySelector) OnResult(res Result) {
	...
	cacheKey := sessionAffinityCacheKey(ns, primaryID, nsModel, res.Options.Metadata)
	var fallbackKey string
	if fallbackID != "" && fallbackID != primaryID && !isSubagentSession(primaryID, fallbackID) {
		fallbackKey = sessionAffinityCacheKey(ns, fallbackID, nsModel, res.Options.Metadata)
	}
	if res.Success {
