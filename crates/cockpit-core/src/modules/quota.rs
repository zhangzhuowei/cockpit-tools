...
pub async fn fetch_project_id_with_context(
    ...
    let result = fetch_project_id_with_fallback(access_token, email, ctx).await;
    (result.project_id, result.subscription_tier, result.credits)
}

async fn fetch_project_id_with_fallback(
    access_token: &str,
    email: &str,
    ctx: &QuotaCloudCodeContext,
) -> ProjectIdFetchResult {
    let urls = cloud_code_candidate_urls(ctx);
    let mut last = ProjectIdFetchResult::default();
    for (index, base_url) in urls.iter().enumerate() {
        let result = fetch_project_id_on_host(access_token, email, ctx, base_url).await;
...
async fn fetch_project_id_on_host(
    access_token: &str,
    email: &str,
    ctx: &QuotaCloudCodeContext,
    base_url: &str,
    ...
    if !skip_cache {
        if let Some(record) = read_api_cache("authorized", email) {
            if is_api_cache_valid(&record) {
                ...
                {
                    let meta = fetch_project_id_with_fallback(access_token, email, ctx).await;
